...
% this code only extracts the waveforms from anything that has been labeled
% a good cluster. 
%
...
fileName = fullfile(gwfparams.dataDir,gwfparams.fileName);  % Since my files are in diff places I do not do this.           
...
unitIDs = unique(gwfparams.spikeClusters);
 [ , good_cluster_idx] = intersect(unitIDs, gwfparams.good_clusters);
...
for curUnitInd=1:numUnits
    curUnitID = (good_cluster_idx(curUnitInd));
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        if spikeTimeKeeps(curUnitInd, curSpikeTime)>length(mmf.Data.x)
            break % goes to next iteration of outer for loop
        end
        tmpWf = mmf.Data.x(1:gwfparams.nCh,...
            spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(1):spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end));
